...
function interpolate(
  ...
  const x1 = Math.min(x0 + 1, fieldWidth - 1);
  const y0 = Math.floor(clampedY / cellHeight);
  const y1 = Math.min(y0 + 1, fieldHeight - 1);

  const tx = (clampedX - x0 * cellWidth) / cellWidth;
  const ty = (clampedY - y0 * cellHeight) / cellHeight;

  const v00 = getter(x0, y0);
  const v10 = getter(x1, y0);
  const v01 = getter(x0, y1);
  const v11 = getter(x1, y1);
...
class Simulation {
  ...
  constructor(width: number, height: number, velocityIterations: number) {
    ...
    //for (let i = 0; i < height; i++) {
      ...
      //this.simulationState.dye[2][i] = 10;
    //}
  ...
  public addCurrent() {
    for (let i = 1; i < this.height; i++) {
      this.simulationState.velocity[1][i] = { left: 50, top: 0 };
    }
  }
